...
따라서 모든 자연수 $n$에 대해 $1$부터 $2n - 1$까지의 홀수로 $[1, n^2]$에서 $2$와 $n^2 - 2$를 제외한 모든 정수를 만들 수 있습니다.
...
$n - 1$개의 컵으로 $h - (2n - 1)$과 $h - 1$ 모두 만들 수 없는 경우는 애초에 해가 없는 경우이므로 다른 경우를 고려하지 않아도 됩니다.

...
$x^2 \equiv 1 \pmod p$에서 $p \mid (x^2 - 1)$이므로 $p \mid (x-1)(x+1)$이고 따라서 성립합니다.

# 밀러-라빈 소수 판별법:

$p$가 짝수가 아닌 소수라고 합시다. 그렇다면 $p-1$은 짝수이고, 자연수 $s$와 홀수 $d$에 대해 $p-1 = 2^sd$와 같이 나타낼 수 있습니다.  
이때 $p$와 서로소인 자연수 $a$에 대해 $a^{p-1} \equiv 1 \pmod p$이므로 $a^{2^sd} \equiv 1 \pmod p$입니다.  
...
- $a^d \equiv 1 \pmod p$
- $a^{2^rd} \equiv -1 \pmod p\ {\sf for\ some}\ 0 \leq r < s$

이때, 명제의 역을 만족시키면 $p$가 소수일 것이라 추측할 수 있습니다. 만약 수가 충분히 작은 경우 몇 개의 $a$에 대해서만 성립을 확인하면 $p$가 소수임을 보장할 수 있습니다.
